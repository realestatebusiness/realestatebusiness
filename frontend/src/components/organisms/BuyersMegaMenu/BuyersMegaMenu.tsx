...
import { buyersMegaMenu, type MegaMenuDefinition } from "../../../data/MegaMenuDefinition";

interface BuyersMegaMenuProps {
  isOpen: boolean;
  onClose: () => void;
  data?: MegaMenuDefinition;
}

const BuyersMegaMenu: React.FC<BuyersMegaMenuProps> = ({
  isOpen,
  onClose,
  data = buyersMegaMenu,
}) => {
  ...
  return (
    <div
      ref={menuRef}
      className="absolute left-1/2 -translate-x-1/2 top-full mt-2 w-[min(90vw,1100px)] bg-white shadow-2xl z-[60] p-6 flex rounded-lg"
      role="menu"
      aria-label="For Buyers menu"
    >
      {/* Left Column */}
      <div className="w-1/4 pr-6 border-r">
        <ul className="space-y-4 text-gray-800 font-semibold text-sm">
          {data.sideCategories.map((cat) => (
            <li key={cat.label} className="flex items-center space-x-2 cursor-pointer hover:text-blue-600">
